...
Yes, I do think this is a useful model because as we can see from the multiple r-squared and the adjusted r-squared, the variability in wins can be effectively explained by the model. The model shows that 'points scored' is a statistically significant factor in impacting (increasing) the number of wins. It also shows yards gained and takeaways weren't statistically significant factors when it comes to the number of wins. 
...
Points Scored has the most impact on wins - there is a 0.71 correlation between points scored and wins, which is the highest out of the three factors. The least impactful appears to be yards gained.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

I could use this model to help explain how different factors have an effect or impact on how many times a team wins. If I am covering a certain team who is doing really bad in their season, I could explain how despite maybe opinion, statisical tests have shown that yard gains actually doesn't have a major or significant impact on the wins of the team; it is actually points scored that has the biggest impact. 