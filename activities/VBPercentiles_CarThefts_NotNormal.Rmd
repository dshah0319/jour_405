...
**Your answer**:
Examples of data that might not be normally distributed: 
- household income within a state -- for example if you took a state like Maryland, the incomes of the state's population probably won't be normally distributed because there are large disparities in income. The mean income could potentially be $50k but in certain affluent areas, people could be making over $200k, skewing the data. 
- medal counts in the olympics -- some major athletes from big countries like a Michael Phelps will take home multiple medals, while other olympic athletes from smaller countries might only take home one medal, which again would skew the data and make it not normally distributed. 

For the example about medal counts, one way a journalist could work with that data is to figure out how much money each country dedicates to their athletes to help explain why some countries and athletes end up taking home more medals than others. 
For the household income example, a journalist could look at the outliers and what jobs people with those outlying incomes have to help explain the data. 
...
**Answer**:
Maryland is in the 95th percentile of service aces, meaning that only 5% of college volleyball teams in the US played more aces than Maryland. They are also scoring more aces than 95% of the teams in the data. 
...
**Answer**:
The distribution is skewed to the left, which is a negative skew. One standard deviation from the mean encapsulates 68% of the data. 
