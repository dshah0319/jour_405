...
Based on simply just looking at the new data in the table, it's tough to tell if scores have improved. If I were to organize the old classroom data and the new data in numerical order, then I would have a better idea if test scores have changed, but by just trying to look at both datasets, it is tough to compare and definitively say if scores have improved or not. 
...
The new mean is 75.75 which is higher than the old mean of 72.6. The new standard deviation is 1.76 which is smaller than the old standard deviation of 4.8. These statistics indicate that students on average are performing better on tests after the implementation of the new program, and the lower standard deviation shows that scores are more consistent as more of the test scores are clustered closer to the mean. 
...
A better title could be, "New Reading Program Improves Classroom Test Averages" 
...
**Hypotheses:**
Null: The new reading program hasn't improved reading test scores
Alternative: The new reading program has improved reading test scores
...
The p-value helps us know the probability of if the null hypothesis is actually true. In this case, it means that the p-value will help us see the probability of if the improvement in reading test scores due to the introduction of the new program or maybe some other factor(s) instead. Since the p-value is 0.000034, this means that there is strong evidence that the hypothesis of the reading program not improving test scores is wrong.   
...
New Program Boosts Student's Reading Scores

After the introduction of a new reading program, the average reading tests scores of third graders shown significant improvement. One month after the program's implenetation, student's standardized reading test scores increased from an average of 72.6 to 75.75. 
...
Beyond test scores, it would be important to track other skills, such as reading comprhension to see if kids are understanding content, and include non-test based measures such as projects or different activties to ensure inclusivity. 
